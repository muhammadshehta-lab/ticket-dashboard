...
# ── 3. سحب ومعالجة البيانات من Google Sheets بداخل تكتل آمن ومغلق بالكامل ─────
...
df_filtered_base = df_raw[(df_raw["Date Only"] >= d_from) & (df_raw["Date Only"] <= d_to)].copy()
if sel_agents: df_filtered_base = df_filtered_base[df_filtered_base["Assigned By"].isin(sel_agents)]

# ── 5. الواجهة الرئيسية والفلاتر التفاعلية الذكية ───────────────────────────────
...
# ✅ بناء وتعريف المتغير المركزي في مكان ثابت لمنع الـ NameError تماماً
df_metrics = df_filtered_base.copy()
...
# ── [A] كروت الأداء العريضة والملونة بتصميم الـ Neon Glow ──────────────────────
...
# ── [B] المخطط الدائري Sunburst المرتب زمنياً بالملي وبجانب بعضها تماماً ───────
